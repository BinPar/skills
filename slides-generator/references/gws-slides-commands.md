...
Quick reference for Google Workspace CLI commands used by the slides-generator skill.
...
```json
{
  "updateTextStyle": {
    ...
    "style": {
      ...
      "foregroundColor": {
        "opaqueColor": {
          "rgbColor": {"red": 0.99, "green": 0.616, "blue": 0}
        }
      },
      "bold": true
    },
...
```json
{
  "createImage": {
    ...
    "elementProperties": {
      ...
      "size": {
        "width": {"magnitude": 306.2, "unit": "PT"},
        "height": {"magnitude": 280.0, "unit": "PT"}
      },
      "transform": {
        "scaleX": 1,
        "scaleY": 1,
...
Creates a new slide from a master layout. Use this for layouts not represented as template slides (SECTION_HEADER, MAIN_POINT, BIG_NUMBER). The `insertionIndex` controls position (0-indexed).

Available predefined layouts: `TITLE`, `SECTION_HEADER`, `TITLE_AND_BODY`, `TITLE_AND_TWO_COLUMNS`, `TITLE_ONLY`, `ONE_COLUMN_TEXT`, `MAIN_POINT`, `SECTION_TITLE_AND_DESCRIPTION`, `CAPTION_ONLY`, `BIG_NUMBER`, `BLANK`.
